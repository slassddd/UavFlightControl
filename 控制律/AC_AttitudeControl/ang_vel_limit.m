 ...
 euler_rad.z=euler_radin(3);
    if (ang_vel_roll_max==0||ang_vel_pitch_max==0)  
        if (ang_vel_roll_max~=0)  
            euler_rad.x = constrain_value(euler_rad.x, -ang_vel_roll_max, ang_vel_roll_max);
        end
        if ( ang_vel_pitch_max~=0)  
            euler_rad.y = constrain_value(euler_rad.y, -ang_vel_pitch_max, ang_vel_pitch_max);
        end
    else  
        ...
        if ( ang_vel_yaw_max~=0)  
            euler_rad.z = constrain_value(euler_rad.z,-ang_vel_yaw_max,+ang_vel_yaw_max);
        end
    end
    euler_rado=[euler_rad.x euler_rad.y euler_rad.z];
